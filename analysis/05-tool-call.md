...
| 阶段 | 做了什么 | 关键调用(文件:行) |
|---|---|---|
| 定型执行身份 | 建立不透明的执行身份、冻结参数快照,并把延迟上下文、取消状态、内容终结器三张表以执行对象为键登记好 | `index.ts:1354`、`index.ts:1856`、`index.ts:1402`、`index.ts:1406`、`index.ts:796-803` |
| 折叠预判 | 被折叠的调用分两种结局:已取消的返回预派发取消,否则返回未知工具加路线提示 | `index.ts:1363-1369`、`index.ts:1419-1421` |
| 参数快照失败 | 参数不是无损 JSON 时提前终止,记成这次调用自己的参数错误 | `index.ts:1402`、`index.ts:1406` |
| 准备段入口 | 前置门开始前先查一次调用者是否已取消 | `index.ts:1453`、`index.ts:1460` |
| 前置钩子瀑布 | 在作用域载具上跑 `tools/pre-execute`,三种结论分别是放行、拒绝、询问 | `index.ts:1465`、`packages/core/scope/src/index.ts:170` |
| 审批 | 询问交给审批服务;没有审批服务或调用不带 agent 都降级为拒绝,一次性放行、被拒、被取消、通道不可用四种文本互不相同 | `index.ts:1469`、`index.ts:1679`、`index.ts:1668-1677` |
| 守卫 | 放行时取守卫理由,拿到理由就按拒绝处理;守卫本身没有"允许"这个返回值 | `index.ts:1109`、`index.ts:1479-1489`、`index.ts:704` |
| 准备段收尾 | 再查一次取消,能走就返回"去执行",不能走就返回待后置的结果 | `index.ts:1490`、`index.ts:1493` |
| 执行段包壳 | `tools/execute` 瀑布包住工具体,这是唯一允许改写信号的阶段 | `index.ts:1559`、`index.ts:1563`、`index.ts:145-154` |
| 融合信号并查取消 | 把调用者信号与配套信号融合;融合后已取消就直接返回预派发取消,不碰工具体 | `index.ts:1522`、`index.ts:1879`、`index.ts:1530` |
| 重新解析定义 | 执行前再解析一次工具定义,准备段之后发生的注册表变化在这里生效 | `index.ts:1536` |
| 调用工具体 | 置"已调用 body"标志后调用工具体;成功但信号已经取消时,结果被换成取消结果 | `index.ts:1538`、`index.ts:1539`、`index.ts:1541` |
| 执行段收尾 | `finally` 里还原执行对象上的信号,后置监听器看到的仍是上游信号 | `index.ts:1547` |
| 结果归一化 | 不是本次执行铸造的结果必须重过输出合同,延迟上下文也在这里合并进来 | `index.ts:1816`、`index.ts:1774`、`index.ts:1829`、`index.ts:1571` |
| 收尾段之分流 | 需要后置处理的先跑 post-execute、再按取消状态替换结果;不需要的直接走同步那条 | `index.ts:1599`、`index.ts:1732`、`index.ts:1604`、`index.ts:1508`、`index.ts:1621` |
...
| 阶段 | 做了什么 | 关键调用(文件:行) |
|---|---|---|
| 通道入口 | 已经有一条通道在跑就直接复用,不会开出第二条;每轮先建好唤醒承诺,免得检查与等待之间到达的事件被丢掉 | `ptc.ts:392`、`ptc.ts:393`、`ptc.ts:400` |
| 提交队首 | 队首已落定就出队并提交,提交走的是完整收尾两段 | `ptc.ts:401`、`ptc.ts:402`、`ptc.ts:403`、`ptc.ts:404` |
| 解除屏障 | 被提交的如果是独占调用,这时才解除独占标志,所以屏障覆盖到提交为止 | `ptc.ts:407`、`ptc.ts:405-407` |
| 取消清理 | 运行控制器已取消时,排队中的调用出队放弃,不落任何日志 | `ptc.ts:412`、`ptc.ts:413`、`ptc.ts:414` |
| 启动时分类 | 启动前重新读一次并发分类,让注册表变化在启动点生效 | `ptc.ts:417`、`ptc.ts:418` |
| 容量判定 | 独占调用要求池完全空,普通调用只要求未到上限,且独占期间一律不放行 | `ptc.ts:419`、`ptc.ts:420` |
| 占位与启动 | 先标记独占,再把调用推进提交队列占住提交位,最后跑准备段 | `ptc.ts:422`、`ptc.ts:424`、`ptc.ts:427`、`ptc.ts:428` |
| 执行入池 | 执行 promise 落定时自行出池并唤醒通道 | `ptc.ts:429`、`ptc.ts:430`、`ptc.ts:431`、`ptc.ts:433` |
| 静默判定 | 待办队列、提交队列、在飞池三者皆空即静默,通道返回 | `ptc.ts:437` |
