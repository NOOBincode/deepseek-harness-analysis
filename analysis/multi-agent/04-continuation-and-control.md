...
可续存子 agent 与一次性子 agent 的**根本差别是"谁持有 `AgentHandle`"**:一次性路线把 handle 包成 `SubagentRun` 交给调用者,一次委托一个结果;可续存路线把 handle **留在 `SubagentContinuationManager` 里**,由它通过子 agent 自己的 inbox 排每一轮。所以 provider 在续存路径上**只贡献一个 `seed` 数据**,看不到 Agent、handle、turn 或 teardown(`subagent/src/types.ts:231-244`)。

日常三件事的实现落点:

| 动作 | 入口 | 内核 |
|---|---|---|
| 建续存子 | `startContinuable`(`continuation.ts:102`) | 预留 id → 描述符快照 → `prepareContinuable` → `materialize` → 投首条 prompt |
| 给子加话 | `sendMessage`(`continuation.ts:202`) | 常驻则 `steer`,否则 `coldResume` |
| 停子当前 turn | `interrupt`(`continuation.ts:332` → `continuation-activation.ts:257`) | 授权检查 → `cancel(cause, {keepInbox: true})` |
...
| ④ | `assertChildIdAvailable`(`continuation-activation.ts:214-218`) | `DUPLICATE_CHILD` |
| ⑤ | `snapshotSubagentDescriptor` | 描述符 JSON 非法 → 在"还没有子 agent"时就拒绝 |
| ⑥ | `holdOwnership`(`:230-250`) | `ACTIVATION_CLOSING`,或返回一个失败路径的 releaser |

第 ⑥ 点的理由写在 `:131-133`:*An idle continuation-managed parent must not settle while a caller is still creating its child. A turn-scoped delegation does not need this, but the service is also callable outside a turn*。`holdOwnership` 返回的 releaser **只删掉本次调用加的那条边**;若期间已经有活的 Activation,所有权边归那个 Activation,releaser 保守保留(`:241-249`)。

### 1.1 关键区里的三重复查

真正的创建发生在 `activations.locks.run(childId, ...)` 里(`:146-184`),并且**在锁内把 ①②④ 全部再查一遍**:
...
`prepareContinuable` 本身也有能力门,由 service 层实现(`subagent/src/index.ts:593-606`):
...
`isAdjacentAgentSendMessageTool`(`internal.ts:32`)判断子 agent 的工具面里**是否真的有 `send_message`**;有才注入"如何回报"的指引(`withContinuableReturnGuidance`,`continuation-messages.ts:81`)。这是"不给它做不到的建议"的写法——`markAdjacentAgentSendMessageTool` 就是给这个判定打的标记(`internal.ts:15,22`)。
...
工具自己的 JSDoc 就把语义界定死了:*They perform no lifecycle routing of their own — residency, cold resume, and interrupt authorization belong to the subagent service*(`index.ts:2-8`)。
...
① 值得强调:用的是**对象身份比较**(`get(id) !== sender`),不是 id 比较。可复用的 id 可能已经解析到一个**替换实例**,所以"精确活体"是授权条件本身。这条规则在 `interrupt`(`continuation-activation.ts:265-270`)和 job 所有权(`jobs-local/src/index.ts:454-456`)里重复出现。
...
最终投递到 inbox 的动作在 registry 里(`continuation-activation.ts:482-503`),它把所有关卡压进一个**没有 await 的临界区**:
...
- `ancestor` 授权走 **`activation.ancestry`**——运行期的活祖先后代集,而不是会话日志里的 `parentSession` 链。所以 `interrupt_agent` 能停"更深一层的、由你起的 agent"(工具描述:*The target may be your direct child or a deeper agent created under you*,`index.ts:79-81`)。
- `{ keepInbox: true }` 是边界的核心。`CancelOptions.keepInbox` 的契约(`packages/core/agent/src/runtime-types.ts:38-45`):*The active turn is still aborted, but un-started and pending work survives for a later turn and no canceled inbox splice is logged*。
- **目标不存在 → 接受的 no-op**(`:279`)。工具描述把这条写进了模型可见文案:*interrupting an agent that already finished is an accepted no-op*。

### 3.3 边界清单

| 停 / 不停 | 行为 | 依据 |
|---|---|---|
| ✅ 停当前 turn | `agent.cancel(cause, {keepInbox:true})` | `continuation-activation.ts:296-299` |
| ✅ 停"关闭截止线"之后不再补救 | `inbox.closing !== undefined` 直接 return | `:295` |
| ❌ 不清 inbox | `keepInbox: true` | `:298` |
| ❌ 不杀它自己起的 agent | Activation 与 `ownedChildren` 完全不动 | `:293-295` 注释;`holdOwnership` 只在 drain 路径释放 |
| ❌ 不返回"已停止" | service 的 `interrupt()` 返回 `void`,工具恒返回 `{accepted:true}` | `subagent/src/index.ts:295-297`;`tool-subagent-control:114` |
| ❌ 不保证立即生效 | 文档原文:*Fire-and-return: the cancel signal is issued before this returns, but the target may keep running until it observes the signal* | `subagent/src/index.ts:282-284` |

停完之后的行为也有定义:*Once the interrupted driver is idle, a waking send resumes the parked FIFO queue*(`subagent/src/index.ts:286-288`)——被保留的排队消息不会丢,下一条 `send_message` 会把它唤醒。
...
2. **`authorizeLineage`**(`continuation-activation.ts:437-451`):父必须是**精确活体**,且 `source.header.parentSession === parent.id`;
3. **`foldSubagentDescriptor(events.slice(inheritedEventCount))`**:只看继承前缀之后的事件——**描述符必须由子自己写下**,不能被 fork seed 里的父描述符冒充;
4. descriptor 必须是 `mode === 'continuable'` 且版本匹配(fold 在版本不符时返回 `undefined`,`descriptor.ts:210`);
...
`statusOf`(`tool-subagent-control/src/list-agents.ts:59-63`)只有三行逻辑:`agents.get(id)` 为 `undefined` → `'ready'`;否则 `agent.status === 'running' ? 'running' : 'idle'`。映射规则(`:53-56`):`running` = 活体且正在工作;`idle` = 活体但在 turn 之间(可能正在等它自己起的 agent);`ready` = 没有活 Agent 了——**仅存在于存储中,可恢复,但不是"待收集的结果"**。

`ready` 的语义被刻意写成"resumable, not terminal"(工具描述原文 `:98-99`),因为一个 `ready` 的子 agent 仍然是合法的 `send_message` 目标——那会走 `coldResume`。所以 list 的输出是**观察**,不是**投递承诺**(`:100-101` 明说 *The snapshot is not a delivery promise*)。

---

## 第六节 `assistant-output.ts`:输出抽取规则
...
| 调用方 | 位置 | 用途 |
|---|---|---|
| 一次性 driver | `subagent-in-process-driver/src/index.ts:226` | `readResult` 取 `output` |
| 生命周期事件 | `subagent/src/lifecycle.ts` | `subagent/end.lastAssistantMessage` |
...
service 面暴露两个方法(`packages/subagent/subagent/src/index.ts:299-330`):**`drainContinuableDescendants(parents)`** 关闭这些父之下整片森林的续存准入 → **同步**停掉可见后代 Activation → 等已承认的 materialization → **child-first** 释放;**`drainContinuableChildren(parent, childIds)`** 只释放选中的常驻直接子,同父的其他子保持驻留。

登记顺序上有一处精心的安排(`continuation-activation.ts:184-198`):
...
| 符号 | 位置 | 职责 |
|---|---|---|
| `SubagentContinuationManager` | `packages/subagent/subagent/src/continuation.ts:82-548` | 续存编排:身份、描述符、provider 准备、冷恢复、授权、路由 |
| `startContinuable` | `continuation.ts:102-190` | 六道前置 + 锁内三重复查 + materialize + 首条 prompt |
| `sendMessage` / `sendToParent` | `continuation.ts:202-232` / `337-360` | 三岔口;发给父恒 `steer`,父不在即 `PARENT_UNAVAILABLE` |
| `deliverToChild` / `deliverFollowup` | `continuation.ts:273-287` / `290-323` | 每 id 一把锁;丢在关闭截止线上则重试 |
| `coldResume` | `continuation.ts:404-454` | observeSession → authorizeLineage → fold → materialize(不派发 provider) |
| `submitMaterialized` / `submitAdmitted` / `assertImageCapable` | `continuation.ts:457-485` / `488-504` / `507-523` | 承认事务;消息构造;投递前查 `inputModalities` |
| `requirePersistence` / `requireSessionQuery` | `continuation.ts:526-535` / `538-547` | `PERSISTENCE_UNAVAILABLE` / `CONTINUATION_UNAVAILABLE` |
| `ContinuableActivationRegistry` | `continuation-activation.ts:153-853` | 进程内 Activation 图:驻留、锁、授权、排空 |
| `assertChildIdAvailable` / `holdOwnership` | `continuation-activation.ts:214-218` / `230-250` | `DUPLICATE_CHILD`;父不可 settle 的边 |
| `interrupt` | `continuation-activation.ts:257-300` | 两条授权阶梯 + `keepInbox: true` |
| `assertAdmitting` / `authorizeLineage` | `continuation-activation.ts:420-429` / `437-451` | `DRAINING`;精确活父 + 直系校验 |
| `materialize` / `submitAdmitted` / `dispose` | `continuation-activation.ts:458-471` / `482-503` / `511-513` | 创建/恢复;无 await 临界区;memoized close |
| `markAdjacentAgentSendMessageTool` / `isAdjacentAgentSendMessageTool` | `subagent/src/internal.ts:15,22,32` | 判定子是否有 `send_message` 能力 |
| `createAgentMessage` / `withContinuableReturnGuidance` | `continuation-messages.ts:62` / `81` | 相邻 Agent 消息与回报指引 |
| `validateControlRequest` / `catalogView` / `rejectPrompt` | `subagent/src/control.ts:40` / `60` / `107` | 远程面入参与错误翻译 |
| `listChildren` / `listDescendants` / `COLD_READ_CONCURRENCY` | `subagent/src/list-children.ts:83-95` / `110-120` / `37` | live-preferred 语料 + 三级阶梯分类 |
| `statusOf` | `tool-subagent-control/src/list-agents.ts:59-63` | `running` / `idle` / `ready` 三态 |
| `send_message` / `interrupt_agent` | `tool-subagent-control/src/index.ts:28-74` / `76-116` | 薄适配:只取 `exec.agent` 与品牌化 id |
| `AssistantOutputFold` / `finalAssistantOutput` | `subagent/src/assistant-output.ts:22-60` / `67-75` | 最后一条非空 assistant 消息 → 累积文本流 |
| `SUBAGENT_DESCRIPTOR_VERSION` / `foldSubagentDescriptor` | `subagent/src/descriptor.ts:48` / `317` | 描述符 v3;版本不符即"不可续存" |
