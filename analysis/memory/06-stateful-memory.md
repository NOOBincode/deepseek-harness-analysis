...
**第一,spill 完全不写会话事件。** 整个 `packages/spill` 里没有任何 `session.append(...)` 调用,`package.json` 与类型层也**没有**对应的 `SessionEventMap` 声明;`known-event-types.ts` 的词表里没有 spill 条目。溢出发生的时刻,日志里被写入的是**工具结果本身的内容被替换成了一段有界文本**,由正常的 `tools/post-execute` 结果路径落盘。
...
| 符号 | 位置 | 作用 |
|---|---|---|
| `GoalOperation`、`GoalChangeMeta`、`GoalMessageSource` | `packages/goal/goal/src/domain.ts:14`、`packages/goal/goal/src/domain.ts:44`、`packages/goal/goal/src/domain.ts:47` | 七个状态变更动词、完整快照或清空墓碑、续跑消息的目标身份归属 |
| `'goal/change'`、`'goal/changed'` 事件声明 | `packages/goal/goal/src/domain.ts:61`、`packages/goal/goal/src/domain.ts:104` | 声明合并进 `SessionEventMap`、进程内的 scoped 通知 |
| `FoldState` / `emptyGoalFoldState` | `packages/goal/goal/src/fold.ts:27` / `:40` | 严格重放的累加器 |
| `decodeGoalChange`、`validateSnapshotTransition` | `packages/goal/goal/src/fold.ts:134`、`packages/goal/goal/src/fold.ts:200` | 变更载荷的严格解码、五个转移各自的前置条件 |
| `applyGoalChange`、`applyGoalEvent`、`foldGoal` | `packages/goal/goal/src/fold.ts:271`、`packages/goal/goal/src/fold.ts:313`、`packages/goal/goal/src/fold.ts:339` | 变更的校验与应用、轮次准入的日志侧校验、从完整日志折叠 |
| `applyGoalProjection`、`goalProjectionDefinition` | `packages/goal/goal/src/index.ts:146`、`packages/goal/goal/src/index.ts:162` | 把严格失败折进状态、key `goal` 且 `stateVersion` 6 |
| `GoalService` | `packages/goal/goal/src/index.ts:240` | 目标服务 |
| `expectCurrent`、`commitSnapshot`、`commit` | `packages/goal/goal/src/index.ts:456`、`packages/goal/goal/src/index.ts:579`、`packages/goal/goal/src/index.ts:609` | CAS 版本校验、完整快照变更构造、追加 + 激活边 |
| `readyToDrive`、`drive` | `packages/goal/goal-round-driver/src/index.ts:103`、`packages/goal/goal-round-driver/src/index.ts:138` | 静默判定、检查点与轮次预约 |
| `validReservation` | `packages/goal/goal-round-driver/src/index.ts:346` | 步进前的准入闸门 |
| `renderGoalRoundPrompt`、`'goal/changed'` 监听 | `packages/goal/goal-round-driver/src/prompt.ts:12`、`packages/goal/goal-round-driver/src/index.ts:283` | 续跑提示的纯渲染、驱动入口 |
| `toTodoList` | `packages/todo/tool-todo/src/index.ts:91` | 工具侧的清单校验 |
| `todos` 投影注册、`todo/write` 追加 | `packages/todo/tool-todo/src/index.ts:134`、`packages/todo/tool-todo/src/index.ts:210` | 整表折叠与回合清空、唯一写入点 |
| `validateTodos` | `packages/todo/tool-todo/src/invariant.ts:24` | 持久形状不变量 |
| `'session/title'` 事件声明、`titleProjectionDefinition`、`foldSessionTitle` | `packages/session/session-title/src/index.ts:71`、`packages/session/session-title/src/index.ts:263`、`packages/session/session-title/src/index.ts:282` | log-only 标题快照、key `title`、权威读回路径 |
| `sessionStatsProjectionDefinition` | `packages/session/session-stats/src/projection.ts:113` | 纯派生统计 |
| `SpillStore.saveText`、`SaveTextSpill` | `packages/spill/spill/src/index.ts:55`、`packages/spill/spill/src/types.ts:63` | 溢写能力缝、归属、来源、建议名、正文 |
| `LocalSpillStore.saveText`、`spillReplacement`、`tools/post-execute` 监听 | `packages/spill/spill-local/src/index.ts:149`、`packages/spill/spill-policy/src/index.ts:125`、`packages/spill/spill-policy/src/index.ts:185` | 落盘实现与取回提示、替换文本构造与预算守住、溢写触发点 |
