...
换成一句话说:调度器的作用是让"必须有序的部分"和"可以并发的部分"各自待在合适的位置上。第一层只回答一个问题——当前这个调用能不能和兄弟一起跑;答不上来(工具未声明、声明抛错、工具不可见)一律按独占处理,这叫失败关闭,宁可慢也不能让不该并发的调用并起来。第二层管一个组的完整生命周期,池子按上限滚动补人,但结果严格按模型给出的顺序提交,所以真正并发的只有执行那一段。第三层是组与组之间的屏障:一个组必须连同它的收尾和日志一起结束,下一组才能开始,独占调用的"独占"正是靠这一层实现的。

![流程图：03-scheduler-and-concurrency](../assets/diagrams/tool-call__03-scheduler-and-concurrency-14.svg)

<details><summary>Mermaid 源码</summary>

```mermaid
flowchart TD
  A["一批模型已经产出的工具调用"] --> B["第一层 分类:当前调用能否与兄弟并发"]
  B -->|可以| C["第二层 分组:整批剩余调用交给滚动池"]
  B -->|不可以| D["第二层 分组:只把这一个单独跑"]
  C --> E["池按上限滚动补充 启动一个就尝试提交一批"]
  D --> E
  E --> F["提交严格按模型给出的顺序逐个推进"]
  F --> G{"这一组还有没开始的调用吗"}
  G -->|有| H["回到第一层 对剩下的调用重新分类"]
  G -->|没有| I["第三层 收尾:本组全部落定并写完日志"]
  I --> J["屏障:下一组的准备动作不会与本组重叠"]
```
...
这段轨迹要说明的其实只有一件事:并发到底发生在哪一节。调度器整体跑在一条单线程的 await 链上,每个调用的准备与提交都排在这条链上,所以任意两次准备之间、任意两次提交之间都不会重叠;唯一真正重叠的是工具体那一段。池上限取 2、组里放 4 个调用时,节奏是这样的:启动第 0 个之后立刻想提交,但它的结果还没回来、槽位空着,于是只能回去启动第 1 个;等第 0 个落定并提交完,池子才腾出位置接纳第 2 个。把上限调小只会拖长这条链,不会破坏顺序。

![时序图：03-scheduler-and-concurrency](../assets/diagrams/tool-call__03-scheduler-and-concurrency-309.svg)
...
sequenceDiagram
  ...
  participant P as 前置门
  participant D as 并发执行
  participant C as 提交与落日志
  L->>P: 第 0 个调用过前置门
  P-->>L: 放行
  L->>D: 启动第 0 个执行 并发
  L->>C: 尝试提交 槽位还空着
  L->>P: 第 1 个调用过前置门 必须等前一个结束
  P-->>L: 放行
  L->>D: 启动第 1 个执行 并发
  D-->>L: 第 0 个执行落定
  L->>C: 提交第 0 个 槽位已填
  ...
  L->>C: 提交第 1 个
  C-->>L: 结果已落日志
  Note over L,D: 前置门与提交互不重叠 只有执行可以并发
```
...
两条准确的说法:**任意两个 `P` 之间、任意两个 `C` 之间都不重叠**(全在 `fillPool`/主循环这条单线程 await 链上);**`D` 与 `P`/`C` 可以重叠**(`D` 是唯一被丢进 `inFlight` 的 promise)。上面折叠起来的那条轨迹只能示意,真实执行中 `P`/`C` 与任何 `D` 都可能交错。
